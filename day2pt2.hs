...
getsubstrings lst
  | null lst = [[]]
  | otherwise = map (\x -> [head lst] ++ x) (getsubstrings (tail lst)) ++ (getsubstrings (tail lst))

allcorrect lst = filter (\x -> length x == (length lst) - 1) $ getsubstrings lst
...
main = do
  ...
  print $ length $ filter id $ map (\x -> any (\y -> allincdic y) x) $ map allcorrect inp
